include("./Funciones/gestorDatosAC_PSO.jl")
include("./Funciones/matrizAdmitancia.jl")


function AC_OPF_PSO(dLinea::DataFrame, dGen::DataFrame, dNodos::DataFrame, nN::Int, nL::Int, bMVA::Int, solver::String, dbin:: Vector{Float64})
    ...
    # dbin      Datos de las binarias definidas en el PSO

    ########## GESTIÓN DE DATOS ##########
    # Asignación de los datos con la función "gestorDatosAC"
    P_Cost0, P_Cost1, P_Cost2, P_Gen_lb, P_Gen_ub, Q_Gen_lb, Q_Gen_ub, S_Demand, V_Nodo_lb, V_Nodo_ub, gen_bus = gestorDatosAC_PSO(dGen, dNodos, nN, bMVA)

    # Matrices de admitancias
    Y, Y_0, Y_Sh = matrizAdmitancia(dLinea, nN, nL)

    # Asigno las posiciones de los generadores al vector de binarias procedente del PSO
    dbin_bus = zeros(Int, nN)

        # j tiene los valores del vector gen_bus, que se encuentra en gestorDatosAC_PSO
    for (i, j) in enumerate(gen_bus) 
        dbin_bus[j] = dbin[i]
    end
    ...
    coste_total = sum(P_Cost0[i] + P_Cost1[i] * P_Gen[i] * dbin_bus[i] * bMVA + P_Cost2[i] * (P_Gen[i] * dbin_bus[i] * bMVA)^2 for i in 1:nN)
    ...
    @constraint(m, [i in 1:nN], real(S_Gen[i]) >= (P_Gen_lb[i]  * dbin_bus[i]))
    @constraint(m, [i in 1:nN], imag(S_Gen[i]) >= (Q_Gen_lb[i]  * dbin_bus[i]))
    
    @constraint(m, [i in 1:nN], real(S_Gen[i]) <= (P_Gen_ub[i] * dbin_bus[i]))
    @constraint(m, [i in 1:nN], imag(S_Gen[i]) <= (Q_Gen_ub[i] * dbin_bus[i]))
    ...
    if termination_status(m) == OPTIMAL || termination_status(m) == LOCALLY_SOLVED || termination_status(m) == ITERATION_LIMIT
        ...
        solBinaria = DataFrames.DataFrame(dbin = value.(dbin_bus[dGen.BUS]))
